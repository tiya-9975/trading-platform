...
const Watchlist = () => {
  ...
  if (watchlist.length === 0) {
    return (
      <div className="p-6">
        <h1 className="text-2xl font-bold text-gray-100 mb-6">My Watchlist</h1>
        <div className="bg-gray-900 rounded-xl p-12 shadow-sm border border-gray-700 text-center">
          <Eye size={48} className="text-gray-600 mx-auto mb-4" />
          <h2 className="text-xl font-semibold text-gray-100 mb-2">No Stocks in Watchlist</h2>
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-3xl font-bold text-gray-100 mb-6">My Watchlist</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {watchlist.map((item) => {
          ...
          return (
            <div key={item._id} className="bg-gray-900 rounded-xl p-6 shadow-sm border border-gray-700 relative group">
              <button
                onClick={() => handleRemove(item.symbol)}
                className="absolute top-4 right-4 p-1 rounded-lg text-gray-500 hover:text-red-400 hover:bg-red-900/20 opacity-0 group-hover:opacity-100 transition-opacity"
              >
                <X size={18} />
              </button>
              ...
              <Link to={`/stock/${item.symbol}`}>
                <div className="mb-4">
                  <h3 className="text-xl font-bold text-gray-100">{item.symbol}</h3>
                ...
                {stock ? (
                  <>
                    <div className="flex items-end justify-between mb-3">
                      <p className="text-2xl font-bold text-gray-100">${stock.price}</p>
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-3 border-t border-gray-700">
                      <div>
                        ...
                        <p className="text-sm font-medium text-gray-300">{stock.volume}</p>
                      </div>
                      <div>
